# RSoT: Robust Generalized Scalar-on-Tensor Regression
...
**RSoT** proposes a robust estimation framework constructed using **maximum $L_q$-likelihood estimation** instead of the classic maximum likelihood estimation. This allows for accurate modeling of multi-dimensional HD input data even when the dataset is contaminated with outliers.
...
### 1. Gaussian Response
* **RSoT_Main_Gaussian.R**: Main execution script for simulations with Gaussian noise.
* **RSoT_Main_Gaussian_Utils.R**: Core utility functions including tensor operations and $L_q$ estimation logic.

### 2. Poisson Response
* **RSoT_Main_Poisson.R**: Main execution script for count-data simulations.
* **RSoT_Main_Poisson_Utils.R**: Specialized functions for handling Poisson likelihood and robust estimation.

### 3. Sparse Regression
* **RSoT_Main_Sparse.R**: Implementation for cases requiring sparsity constraints.
* **RSoT_Main_Sparse_Utils.R**: Utility functions for the sparse modeling framework.
  
## Getting Started

1. **Download the Scripts**: Ensure all `.R` files (both Main and Utils) are stored in the same directory.
2. **Set Your Paths**: Open the **Main** R script you wish to run (e.g., `RSoT_Main_Gaussian.R`).
...
If you use this code or the RSoT method in your research, please cite our paper:

```bibtex
@article{konyar2024RSoT,
