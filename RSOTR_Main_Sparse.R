...
source(here("RSOTR_Main_Sparse_Utils.R"))
RunSet = 1
...
for(runId in 1:30){
  ...
  #------------------------> Convergence------------------------
  local_iter_max=10
  local_iter_max_alpha_selection=20
  local_iter_max_main=20
  stopping_cond_L_init=0.000001
  stopping_cond_L_alpha_selection=0.000001
  stopping_cond_L_local_main=0.000001
  ####################----- DATA GENERATION-----####################
  ####################-----> X_it-----####################
  ...
  ####################-----> Y_t-----####################
  B_groundtruth=sotr_generate_B_sparse(u_t_mean=sotrl_u_t_mean,u_t_sigma=sotrl_u_t_sigma
  ...
  Y_t_list=r_gsotr_generate_Y_t(sigma=sotrl_y_sigma,mu=sotrl_y_mu,number_of_datasets
                                ,number_of_modes,U_dtr_list=NA,TensorX_ti_train_list
  ...
  stdev_est=ceiling(sqrt(max(Y_t[[1]])/6))
      ...
      for(rank_iter in rank_list){
        ...
        for(alpha_iter in alpha_list){
          ...
          full_iter=r_gsotr_local_iteration(h_y,b_theta,b_deriv,b_deriv2,theta_vec,stdev_est,b_sigma
                                            ...
                                            ,scale=1,calculate_r_gsotr_loss,U_dtr_list_all)
  ...
  toc()
  alpha_main3=median_median_residual[which.min(rmse_trunc3)]
  ...
  total_iter_number=1
  Mu_t=Mu_t_all[[which(rank_list==unique(alpha_main3$R))]]
  U_dtr_list=U_dtr_list_all[[which(rank_list==unique(alpha_main3$R))]]
  predictions_tab_main_train=data.table()
  predictions_tab_main_test=data.table()
  R=unique(alpha_main3$R)
  for(alpha_selected in unique(c(alpha_main3$alpha))){ 
    for(t in 1:number_of_datasets){
      full_iter=r_gsotr_local_iteration(h_y,b_theta,b_deriv,b_deriv2,theta_vec,stdev_est,b_sigma,b_deriv_sigma
                                        ...
                                        ,TensorX_ti_train_list,TensorX_ti_test_list,t,Y_t,Y_t_test,Mu_t,N,N_test,R
      ...
      performance_table=r_gsotr_add_iter_to_performance_table(performance_table,model_type="R_GSOTR" 
                                                              ...
                                                              ,class_train,class_test,R,alpha_selected
                                                              ,t,prob_list[[t]],prob_list_test[[t]],epsilon_list[[t]]
                                                              ,epsilon_list_test[[t]],scale=1)
  ...
  ####################----- BENCHMARK GSOTR -----####################
  local_iter_max=10#15#25
  stopping_cond_L_init=0.000001#0.0001#0.000001
  print(paste0("Run ",runId," initialization:"))
  tic()
  summary_table_L_LOCALMODEL=data.table()
  AIC_summary=data.table()
  ...
  for(rank_iter in rank_list){
    ...
    for(t in 1:number_of_datasets){
      Mu_t=0
      full_iter=gsotrSparse_local_iteration(b_theta,b_deriv,b_deriv2,theta_vec,stdev_est,eta_deriv_sigma
                                            ,eta_deriv2_sigma,b_deriv_sigma,b_deriv2_sigma,U_dtr_list
                                            ,TensorX_ti_train_list,TensorX_ti_test_list,t,Y_t,Y_t_test,Mu_t
                                            ,N,N_test,rank_iter,dimensions,stopping_cond_L=stopping_cond_L_init
                                            ,summary_table_L_LOCALMODEL,total_iter=total_iter_number
                                            ,local_iter_max=local_iter_max,main_iter=1,distribution
                                            ,is_scaled,number_of_modes,number_of_datasets,epsilon_list
                                            ,epsilon_list_test,prob_list,prob_list_test,scale=1,calculate_gsotr_loss,U_dtr_list_all)
      summary_table_L_LOCALMODEL=full_iter[[1]]
      total_iter_number=full_iter[[2]]
      U_dtr_list=full_iter[[4]]
      Mu_t=full_iter[[5]]
  ...
  fwrite(AIC_summary,paste0("BENCHMARK_sparseGSOTR_REGRESSION_AIC_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(performance_table,paste0("BENCHMARK_sparseGSOTR_REGRESSION_perfomance_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(summary_table_L_LOCALMODEL,paste0("BENCHMARK_sparseGSOTR_REGRESSION_summary_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(predictions_tab_train,paste0("BENCHMARK_sparseGSOTR_REGRESSION_predictions_train_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(predictions_tab_test,paste0("BENCHMARK_sparseGSOTR_REGRESSION_predictions_test_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  toc()
  ...
  cv_hubers=cv.hqreg(seed=1,X=as.matrix(X_linear_train,nrow=315,ncol=256),y=Y_linear_train,FUN="hqreg_raw",type.measure = "mse")
  performance_table=data.table()
  predictions_tab=data.table()
  Y_predicted=predict(cv_hubers,as.matrix(X_linear_train,nrow=315,ncol=256))
  Y_predicted_test=predict(cv_hubers, as.matrix(X_linear_test,nrow=315,ncol=256))
  ...
  fwrite(performance_table,paste0("BENCHMARK_lassoPCA_RLRHUBER_GAUSSIAN_perfomance_",distribution,"_percent100_RunSet",RunSet,"_runID",runId,".csv"))
  preds=data.table(Actual_test=as.numeric(Y_linear_test),Predicted_test=as.numeric(Y_predicted_test))
  fwrite(preds,paste0("BENCHMARK_lassoPCA_RLRHUBER_GAUSSIAN_predstest_",distribution,"_percent100_RunSet",RunSet,"_runID",runId,".csv"))
