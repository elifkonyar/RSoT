...
source(here("RSOTR_Main_Poisson_Utils.R"))
RunSet = 1
...
for(runId in 1:number_of_runs){
  ...
  sotrl_u_t_mean=0
  sotrl_u_t_sigma=0.5
  sotrl_ground_truth_R=2
  poisson_true_intercept=1.5
  #--initialize 
  sotrl_init_u_t_mean=0
  sotrl_init_u_t_sigma=0.5
  ...
  poisson_multiplier=0.1 
  is_scaled=F
  
  b_theta=copy(poisson_b)
  b_deriv=copy(poisson_b_deriv)
  b_deriv2=copy(poisson_b_deriv2)
  h_y=copy(poisson_h_mean)
  #------------------------> Outliers------------------------
  outlier_q=0.1
  outlier_lambda=3
  add_outlier=T
  ...
  maxval=4
  dividend=2
  alpha_list=c(maxval/dividend,maxval/dividend^(3/2),maxval/dividend^(4/2),maxval/dividend^(5/2),maxval/dividend^(6/2),maxval/dividend^(7/2))
  rank_list=c(2,3,4)
  ...
  ####################-----> Y_t-----####################
  U_dtr_groundtruth=sotr_generate_U_dtr(u_t_mean=sotrl_u_t_mean,u_t_sigma=sotrl_u_t_sigma
  ...
  Y_t_list=r_gsotr_generate_Y_t(sigma=sotrl_y_sigma,mu=sotrl_y_mu,number_of_datasets
                                ...
                                ,poisson_multiplier,outlier_lambda,poisson_true_intercept)
  
  Y_t=Y_t_list[[1]]
  prob_list=Y_t_list[[2]]
  epsilon_list=Y_t_list[[3]]
  lambda=Y_t_list[[4]]
  minimum_error_train=calculate_RMSE(Y_t[[1]],lambda)
  minimum_error_train_weighted=calculate_weightedRMSE(Y_t[[1]],lambda)
  minimum_logscore_train=calculate_logscore(Y_t[[1]],lambda)
  ...
  lambda_test=Y_t_list_test[[4]]
  minimum_error_test=calculate_RMSE(Y_t_test[[1]],lambda_test)
  minimum_error_test_weighted=calculate_weightedRMSE(Y_t_test[[1]],lambda_test)
  minimum_logscore_test=calculate_logscore(Y_t_test[[1]],lambda_test)
  
  ####################----- ALPHA SELECTION -----####################
  print(paste0("Run ",runId," cross validation:"))
  tic()
  set.seed(num_of_folds+RunSet*runId)
  predictions_test_cv_list=data.table()
  ...
  for(t in 1:number_of_datasets){
    ...
    for(fold_iter in 1:num_of_folds){
      ...
      for(rank_iter in rank_list){
        ...
        U_dtr_list_cv_init=initialize_u_l2(u_t_mean=sotrl_init_u_t_mean,u_t_sigma=sotrl_init_u_t_sigma
        ...
        Mu_t_cv_init=gsotr_update_mu(b_theta,b_deriv,b_deriv2,Mu_t_cv_init,number_of_datasets
                                     ,number_of_modes,U_dtr_list_cv_init,cv_train_TensorX_ti_list,cv_train_N
                                     ,cv_train_Y_t,t,stopping_cond=0.000001,calculate_gsotr_loss,learning_rate_init=1)
        summary_table_L_LOCALMODEL=data.table()
        total_iter_number=1
        full_iter=gsotr_local_iteration(b_theta,b_deriv,b_deriv2,U_dtr_list_cv_init
                                        ...
                                        ,calculate_param_dev_tensor,U_dtr_groundtruth,learning_rate_u=1,learning_rate_mu = 1)
        ...
        for(alpha_iter in alpha_list){
          ...
          full_iter=r_gsotr_local_iteration(h_y,b_theta,b_deriv,b_deriv2,U_dtr_list ,cv_train_TensorX_ti_list
                                            ,cv_test_TensorX_ti_list,t,cv_train_Y_t,cv_test_Y_t,Mu_t
                                            ,cv_train_N,cv_test_N,rank_iter,dimensions
                                            ,stopping_cond_L=stopping_cond_L_alpha_selection#[alpha_iter_id]
                                            ...
                                            ,scale=1,calculate_r_gsotr_loss,calculate_param_dev_tensor,U_dtr_groundtruth
  ...
  predictions_tab=data.table()
  for(rank_iter in unique(alphaR_main3$R)){
    Mu_t=rep(0,number_of_datasets)
    U_dtr_list=initialize_u_l2(u_t_mean=sotrl_init_u_t_mean,u_t_sigma=sotrl_init_u_t_sigma
                               ,dimensions,init_tensor_R=rank_iter,rank_iter,number_of_modes,number_of_datasets
                               ,sotrl_initialize_udtr_seed,sotrl_initialize_hosvd_seed
    ...
    for(t in 1:number_of_datasets){
      Mu_t=gsotr_update_mu(b_theta,b_deriv,b_deriv2,Mu_t,number_of_datasets
                           ...
                           ,stopping_cond=0.000001,calculate_gsotr_loss,learning_rate_init =1)
      full_iter=gsotr_local_iteration(b_theta,b_deriv,b_deriv2,U_dtr_list
      ...
      U_dtr_list=full_iter[[4]]
      Mu_t=full_iter[[5]]
      AIC_summary=rbind(AIC_summary,data.table(Site=t,R=rank_iter,AIC=full_iter[[6]]
                                               ,AIC_error=full_iter[[7]]
                                               ,AIC_param=full_iter[[8]]
                                               ,BIC=full_iter[[9]]
                                               ,BIC_error=full_iter[[10]]
                                               ,BIC_param=full_iter[[11]]
                                               ,L=full_iter[[3]]))
      ...
      predictions_tab_train=rbind(predictions_tab_train,data.table(R=rank_iter
                                                                   ...
                                                                   ,Lambda_train=as.numeric(lambda)
      ))
      predictions_tab_test=rbind(predictions_tab_test,data.table(R=rank_iter
                                                                 ...
                                                                 ,Lambda_test=as.numeric(lambda_test)
      ))
      ...
      performance_table=r_gsotr_add_iter_to_performance_table(performance_table,model_type="R_GSOTR_INIT",Y_t[[t]]
                                                              ...
                                                              ,epsilon_list_test[[t]],scale=1,lambda,lambda_test)
    }
  ...
  fwrite(AIC_summary,paste0("R_GSOTR_POISSON_init_AIC_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(performance_table,paste0("R_GSOTR_POISSON_init_perfomance_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(summary_table_L_LOCALMODEL,paste0("R_GSOTR_POISSON_init_summary_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(predictions_tab_train,paste0("R_GSOTR_POISSON_init_predictions_train_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(predictions_tab_test,paste0("R_GSOTR_POISSON_init_predictions_test_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  toc()
  ...
  predictions_tab_main=data.table()
  for(alpha_selected in unique(alphaR_main3$alpha)){
    Mu_t=copy(Mu_t_init)
    U_dtr_list=copy(U_dtr_list_init)
    for(t in 1:number_of_datasets){
      full_iter=r_gsotr_local_iteration(h_y,b_theta,b_deriv,b_deriv2,U_dtr_list,TensorX_ti_train_list,TensorX_ti_test_list
                                        ,t,Y_t,Y_t_test,Mu_t,N,N_test,min(alphaR_main3$R),dimensions,stopping_cond_L=stopping_cond_L_local_main
                                        ,summary_table_L_R_GSOTR,total_iter=total_iter_number,local_iter_max=local_iter_max_main
                                        ,main_iter=1,alpha_selected,distribution,is_scaled,number_of_modes,number_of_datasets
                                        ,prob_list,prob_list_test,epsilon_list,epsilon_list_test,scale=1
                                        ,calculate_r_gsotr_loss,calculate_param_dev_tensor,U_dtr_groundtruth
                                        ,learning_rate_u=1,learning_rate_mu = 1)
  ...
  performance_table[,min_err_train:=minimum_error_train]
  performance_table[,min_err_test:=minimum_error_test]
  performance_table[,min_err_train_weighted:=minimum_error_train_weighted]
  performance_table[,min_err_test_weighted:=minimum_error_test_weighted]
  performance_table[,min_err_train_logscore:=minimum_logscore_train]
  performance_table[,min_err_test_logscore:=minimum_logscore_test]
  fwrite(performance_table,paste0("R_GSOTR_SIMRUNS_main_perfomance",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(summary_table_L_R_GSOTR,paste0("R_GSOTR_SIMRUNS_main_summary",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  ...
  fwrite(AIC_summary,paste0("BENCHMARK_GSOTR_POISSON_AIC_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(performance_table,paste0("BENCHMARK_GSOTR_POISSON_perfomance_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(summary_table_L_LOCALMODEL,paste0("BENCHMARK_GSOTR_POISSON_summary_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(predictions_tab_train,paste0("BENCHMARK_GSOTR_POISSON_predictions_train_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  fwrite(predictions_tab_test,paste0("BENCHMARK_GSOTR_POISSON_predictions_test_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  toc()
  ...
  performance_table=data.table()
  predictions_tab=data.table()
  for(capped_value in c(1,0.1,0.001,0.00001,0.0000001)){
    performance_table_iter=data.table()
    Y_predicted=predict(model_fit,newdata=X_pca_train)
    Y_predicted_capped=ifelse(Y_predicted<=0,capped_value,Y_predicted)
    Y_predicted_test=predict(model_fit, newdata=X_pca_test)
    Y_predicted_test_capped=ifelse(Y_predicted_test<=0,capped_value,Y_predicted_test)
    
    performance_table_iter=r_gsotr_add_iter_to_performance_table(performance_table_iter,model_type="PCA_RLRHuber" 
                                                                 ,Y_linear_train,Y_predicted_capped
                                                                 ,Y_linear_test,Y_predicted_test_capped
                                                                 ,summary_table_L_lastiters=NA
                                                                 ,round=1,N,N_test,number_of_datasets
                                                                 ,distribution = distribution
                                                                 ,class_train=NA,class_test=NA,R="vector",alpha=NA
                                                                 ,t,p_t=NA,p_t_test=NA,epsilon=NA
                                                                 ,epsilon_test=NA,scale=1
                                                                 ,as.numeric(lambda),as.numeric(lambda_test))
    performance_table_iter[,capped_val:=capped_value]

    performance_table=rbind(performance_table,performance_table_iter)
  }
  preds=data.table(Actual_test=as.numeric(Y_linear_test),Predicted_test=as.numeric(Y_predicted_test))
  fwrite(preds,paste0("BENCHMARK_PCA_RLRHUBER_predstest_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))  
  fwrite(performance_table,paste0("BENCHMARK_PCA_RLRHUBER_POISSON_perfomance_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  ...
  for(fold_iter in 1:num_of_folds){
    ...
    for(gamma_iter in c(0.00001,0.0001,0.001,0.005,0.01,0.05,0.1,0.5)){
      ...
      # print(calculate_RMSE(Y_linear_train,Y_predicted))
  ...
  Y_predicted_capped=ifelse(Y_predicted<=0,1,Y_predicted)
  Y_predicted_test_capped=ifelse(Y_predicted_test<=0,1,Y_predicted_test)
  ...
  fwrite(performance_table,paste0("BENCHMARK_EXPSQLOSS_POISSON_perfomance_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))
  preds=data.table(Actual_test=as.numeric(Y_linear_test),Predicted_test=as.numeric(Y_predicted_test))
  fwrite(preds,paste0("BENCHMARK_EXPSQLOSS_POISSON_predstest_",distribution,"_RunSet",RunSet,"_runID",runId,".csv"))  
